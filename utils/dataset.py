...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary):
        ...
        self.img_id2idx = _pickle.load(open(os.path.join(config.ids_path, '{}36_imgid2idx.pkl'.format(image_split)), 'rb'))
        self.h5_path = os.path.join(config.rcnn_path, '{}36_attribute_plus_label.h5'.format(image_split))
        if config.in_memory:
    ...
    def load_image(self, image_id):
        ...
        #img_w = torch.tensor(self.image_feat['img_w'][idx]).view(1, 1)
        #img_h = torch.tensor(self.image_feat['img_h'][idx]).view(1, 1)
        #wh = torch.cat([img_w, img_h]).squeeze(-1)
